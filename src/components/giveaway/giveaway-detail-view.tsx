...
import { getPublicWinners, type PublicGiveawayDetail } from "@/server/queries/public-giveaway";
...
/**
 * GiveawayDetailView — the public giveaway page body: banner, badges, prize,
 * stats, countdown, winners, details, project links, and the entry wizard.
 *
 * Shared by oxbot's `/giveaways/[slug]` and OxFoxes Collab's
 * `/raffles/[slug]` (a public whitelist raffle IS a giveaway), so both
 * products render and enter raffles identically. Chrome (header/footer) is
 * the caller's.
 */
export async function GiveawayDetailView({
  giveaway,
  viewerId,
  returnPath,
  aboveTitle,
}: {
  giveaway: PublicGiveawayDetail;
  viewerId: string | null;
  /** Where sign-in / account linking returns to. */
  returnPath: string;
  /** Optional context rendered above the badges (e.g. the Collab listing). */
...
function ProjectLinks({ giveaway }: { giveaway: PublicGiveawayDetail }) {
